...
TArray<FString> UDLCLoader::GetClassesToLoad()
{
	...
	UAssetManager* assetManager = &UAssetManager::Get();
	assetManager->GetAssetRegistry().GetDerivedClassNames(baseClasses,
		exludedClasses, derivedClasses);
	for (FName derived : derivedClasses)
	{
		...
		if (derivedString.EndsWith("_C")) derivedString = derivedString.Mid(0, derivedString.Len() - 2);
